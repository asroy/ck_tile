...
// FMHA_FWD_KERNEL_DEFINE(fmha_kernel_{F_idx})
...
class KernelFlatData:
    ...
    def name(self) -> str:
        ...
        return f"d{self.F_hdim}_{self.F_dtype}_{self.F_mode}_b{self.F_bm0}x{self.F_bn0}x{self.F_bk0}x{self.F_bn0}x{self.F_bk1}x{self.F_bk0blen}" +\
        f"_r{self.F_rm}x{self.F_rn}x{self.F_rk}_w{self.F_wm}x{self.F_wn}x{self.F_wk}_v{self.F_vlayout[0]}" +\
        f"_p{BOOL_MAP[self.F_m0pad][0]}{BOOL_MAP[self.F_m0k1pad][0]}{BOOL_MAP[self.F_k0n1pad][0]}_{BOOL_MAP[self.F_bias][0]}"  +\
        f"_o{self.F_occupancy }_m{self.F_mask[0]}_{self.F_pipeline}"

@dataclass
class KernelFlat:
    direction: str
    hdim: int
    f_data: KernelFlatData
...
def get_all_kernels() -> List[Kernel]:
    ...
    for mode, mask, bias in itertools.product(MODE_MAP.keys(), MASK_MAP.keys(), ["t", "f"]):
        ...
        #  yield KernelFlat("fwd",  32, KernelFlatData(0,  "fp8", 128,  64,  16,  32,  32,  32, 2, 1, 1, 32, 32, 16, "col", "f", "f", "f", "f", 2   ,mask, mode, "qr_async_fp8"))
        # yield KernelFlat("fwd",  64, KernelFlatData(0,  "fp8", 128,  64,  32,  64,  32,  64, 4, 1, 1, 32, 32, 16, "col", "f", "f", "f", "f", 2   ,mask, mode, "qr_async_fp8"))
        yield KernelFlat("fwd", 128, KernelFlatData(0,  128, "fp8", 128, 128,  32, 128,  32, 128, 4, 1, 1, 32, 32, 32, "col", "f", "f", "f", bias , 2   ,mask, mode, "qr_fp8"))
